...
def compute_L(yhat, y):
	...
	for i in range(len(yhat)):
		error_squared += (yhat.item(i) - y.item(i)) ** 2
	L = error_squared / (len(yhat) * 2)
	return L
...
def compute_dL_dw(y, yhat, Phi):
	...
	for feature in range(p):
		for sample in range(n):
			dL_dw_w += (yhat.item(sample) - y.item(sample)) * Phi.item(sample, feature)
		# Assign and reset
		dL_dw.append(dL_dw_w / n)
		dL_dw_w = 0
	dL_dw = np.matrix(dL_dw).reshape(p, 1)
	return dL_dw
