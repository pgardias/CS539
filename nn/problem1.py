...
def compute_a(z):
    ...
    '''
    # x = np.matrix.copy(z)
    # x[x > 700.0] = 700  # these round huge numbers down or small nubmers up to the min/max exp can run
    # x[x < -700.0] = -700
    # a = x / np.sum(x)
    a = []
    sumz = 0
...
def compute_L(a, y):
    ...
    a0 = np.copy(a)
    a0[abs(a0) < 1e-7] = 1
    z = np.zeros((np.shape(a)[0], 1))
    z[y][0] = 1
    t = np.multiply(z, np.log(a0))
    L = float(-1 * np.sum(t))
    if abs(a[y][0] < 1e-7):
        L = 1e7
    return L
...
def compute_dL_da(a, y):
    ...
    '''
    dL_da = np.matrix(np.zeros(a.shape))
    if abs(a[y][0]) < 1e-7:
        dL_da[y][0] = - 1e7
    else:
        dL_da[y][0] = -1 / a[y][0]
...
def compute_da_dz(a):
    ...
    x = a.shape[0]
    a = np.array(a)
    d = np.mat(np.zeros(x ** 2)).reshape((x, x))
    for i in range(x):
        for j in range(x):
            if i == j:
                d[i, j] = a[i] * (1 - a[i])
...
def compute_dz_dW(x, c):
    ...
    '''
    dz_dW = np.zeros((c, 1), dtype=x.dtype) + np.squeeze(x)
    return dz_dW
...
def predict(Xtest, W, b):
    ...
    for i, x in enumerate(Xtest):
        x = x.T  # convert to column vector
        P[i] = np.squeeze(compute_a(np.matmul(W, x) + b))
